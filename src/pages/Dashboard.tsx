...
const Dashboard: React.FC = () => {
  ...
  // KPI calculations
  const totalCollected = todaysReceipts.reduce((sum, receipt) => sum + receipt.totalAmount, 0);
  
  const totalQuantity = todaysReceipts.reduce((sum, receipt) =>
    sum + receipt.items.reduce((itemSum, item) => itemSum + item.quantity, 0), 0
  );
  
  const distinctClients = new Set(todaysReceipts.map(receipt => receipt.clientId)).size;
  ...
  const handleViewPDF = async (receipt: Receipt) => {
    if (!companyDetails) {
      toast.error('Szczegóły firmy nie zostały załadowane. Proszę spróbować ponownie.');
      return;
    }
  ...
  if (!user) {
    return (
      <div className="flex items-center justify-center h-64">
        <p className="text-gray-500">Proszę się zalogować aby zobaczyć panel.</p>
      </div>
    );
  }
  ...
  return (
    ...
    <div>
          ...
          <p className="mt-2 text-gray-600">
            Dzisiejszy przegląd - {new Date().toLocaleDateString('pl-PL', { 
              weekday: 'long', 
              year: 'numeric', 
              month: 'long', 
              day: 'numeric' 
        ...
        <button
          ...
          className="px-4 py-2 text-sm font-medium text-white bg-orange-700 rounded-md hover:bg-orange-800"
        >
          Nowy Kwit
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wider">Wydano</h3>
          <p className="mt-2 text-3xl font-bold text-green-600">{formatCurrency(totalCollected)}</p>
        </div>
        
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wider">Zebrano</h3>
                          <p className="mt-2 text-3xl font-bold text-orange-700">{totalQuantity.toLocaleString('pl-PL', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} kg</p>
        </div>
        
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wider">Klienci</h3>
          <p className="mt-2 text-3xl font-bold text-purple-600">{distinctClients}</p>
        </div>
        
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wider">Kwity</h3>
          <p className="mt-2 text-3xl font-bold text-orange-600">{totalReceipts}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-800">Dzisiejsze Kwity</h2>
          <p className="text-sm text-gray-600">{todaysReceipts.length} kwit{todaysReceipts.length === 1 ? '' : todaysReceipts.length < 5 ? 'y' : 'ów'} dzisiaj</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Numer Kwitu</th>
                ...
                <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Łączna Kwota</th>
                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Akcje</th>
              </tr>
            </thead>
              ...
              {loading ? (
                <tr>
                  <td colSpan={5} className="text-center py-8">
                    <div className="flex items-center justify-center">
                      ...
                      <span className="ml-2">Ładowanie dzisiejszych kwitów...</span>
                    </div>
                  </td>
                </tr>
                ...
                todaysReceipts.map((receipt) => (
                  <React.Fragment key={receipt.id}>
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{formatTime(receipt.date)}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{getClientName(receipt)}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right font-medium">{formatCurrency(receipt.totalAmount)}</td>
                              ...
                              <table className="min-w-full bg-white rounded-lg shadow-sm">
                                <thead className="bg-gray-100">
                                  <tr>
                                    ...
                                    <th className="text-right py-2 px-3 text-xs font-medium text-gray-500 uppercase tracking-wider">Cena Sprzedaży</th>
                                  ...
                                  {receipt.items.map((item, index) => (
                                    <tr key={index}>
                                      ...
                                      <td className="py-2 px-3 text-sm text-gray-900 text-right">
                                        {item.quantity.toLocaleString('pl-PL', { minimumFractionDigits: 1, maximumFractionDigits: 1 })} {item.unit}
                                      </td>
                                      <td className="py-2 px-3 text-sm text-gray-900 text-right">{formatCurrency(item.buy_price)}</td>
                                      <td className="py-2 px-3 text-sm text-gray-900 text-right">{formatCurrency(item.sell_price)}</td>
                ...
                <tr>
                  <td colSpan={5} className="text-center py-8 text-gray-500">
                    Nie utworzono dzisiaj żadnych kwitów. Kliknij "Dodaj Kwit" aby utworzyć pierwszy kwit dzisiaj.
                  </td>
                </tr>
              )}
            </tbody>
