...
import { FaTachometerAlt, FaUsers, FaBox, FaFileInvoice, FaChartBar, FaCog, FaPlus } from 'react-icons/fa';
...
const NavItem = ({ to, icon: Icon, label }: { to: string, icon: IconType, label: string }) => {
  const commonClasses = "flex items-center justify-center p-4 text-gray-400 hover:text-white hover:bg-gray-600 transition-colors duration-200 group relative";
  ...
  return (
    ...
    >
      ...
      <span className="absolute left-full ml-4 px-2 py-1 text-sm text-white rounded opacity-0 group-hover:opacity-100 transition-opacity duration-300 whitespace-nowrap z-50" style={{backgroundColor: '#2D2D2D'}}>
        {label}
      </span>
    </NavLink>
  );
...
const MainLayout: React.FC = () => {
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      ...
      <OfflineDataHandler />
      <SyncIndicator />
      <div className="w-20 shadow-md transition-all duration-300 flex flex-col" style={{backgroundColor: '#2D2D2D'}}>
          ...
          <div className="mt-auto mb-4">
            ...
            >
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15m3 0l3-3m0 0l-3-3m3 3l-3 3" />
              </svg>
              <span className="absolute left-full ml-4 px-2 py-1 text-sm text-white rounded opacity-0 group-hover:opacity-100 transition-opacity duration-300 whitespace-nowrap z-50" style={{backgroundColor: '#2D2D2D'}}>
                Wyloguj
              </span>
